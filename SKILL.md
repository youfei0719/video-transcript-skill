...
8. **Finish at the evidence boundary.** `verified` means the reviewed transcript is sufficiently supported by the available source evidence. `needs_review` means a human must resolve listed uncertainties. `blocked` means the actual media/text could not be obtained or the source is not authorized. In a blocked result, label both manuscript fields `unavailable`, provide one of `AUTHORIZATION_UNCLEAR`, `NO_ADAPTER`, or `NO_TRANSCRIPT_EVIDENCE`, and state the smallest next action that would supply real evidence.
