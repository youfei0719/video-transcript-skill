...
description: Extract a real video manuscript with Codex-native media understanding when available, context-correct it, and return a naturally paragraphed reviewed version with evidence-bounded corrections. Do not require BaoCut, Whisper, local models, or other user-installed ASR software; do not use for creative rewriting, publishing, or platform-access bypass.
metadata:
...
2. **Choose the available adapter.** Prefer user-supplied subtitles/transcript. Next use Codex-native audio/video understanding on media the session can actually inspect (an attached file, supported workspace media, or an authorized media connector). For a public Douyin URL, the bundled anonymous Chromium adapter (`scripts/download_public_douyin.py`) may obtain a temporary media file, but downloading alone is not transcription: pass it to a Codex-native media capability only when the current session supports that handoff. If an existing BaoCut CLI is already present, it may be detected and reused through `scripts/transcribe_with_existing_baocut.py`; this is strictly optional and must never trigger installation instructions, model downloads, or settings changes. Do not ask users to install BaoCut, Whisper, a local model, or any other ASR package. An external ASR service is an optional, explicitly user-approved fallback; if no native capability, supplied text, existing optional adapter, or approved fallback is available, report `blocked` and state exactly what evidence is missing.
